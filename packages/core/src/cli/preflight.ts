...
/**
 * Make sure a workspace has everything its decks need before serving them.
 *
 * A deck is portable code, but what it *renders* with is not: the fonts it
 * names and the assets it imports live outside the file. Someone opening a deck
 * a colleague wrote would otherwise get silent substitutions — worst of all in
 * the editable PPTX export, which names fonts by family.
 *
 * So on every dev-server start we read the decks, work out what they need,
 * install what we can, and report the rest. Each deck opened this way leaves
 * the machine a little better equipped than it found it.
 */
...
/** Generic CSS keywords are never a real family. */
...
/** A family counts as present if it, or any of its named faces, is installed. */
