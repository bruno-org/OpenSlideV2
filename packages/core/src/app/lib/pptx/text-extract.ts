/**
 * Pulls editable text out of a rendered slide page.
 *
 * The editable PPTX export paints the page as a background image with every
 * promoted text made transparent, then lays native text boxes on top. This
 * module decides what can be promoted and measures exactly where the browser
 * put each line, so the text box lands on the same pixels.
 *
 * Anything we can't reproduce faithfully as PowerPoint text (gradient-filled
 * text, text shadows, rotated or clipped text) is simply not promoted: it stays
 * baked into the background image, so the slide still looks right.
 */

export type TextRun = {
  text: string;
  /** Family as the browser resolved it, used for measuring. */
  fontFamily: string;
  /** Family name written into the PPTX, already resolved for weight. */
  typeface: string;
  /** Whether the PPTX run carries the bold flag. */
  bold: boolean;
  /** Family to measure against when calibrating (the substitute, if any). */
  measureFamily: string;
  /** Weight to measure with; a named face already carries its own weight. */
  ...
  /** Hex without '#'. */
  color: string;
  /** 0..1, already multiplied by inherited opacity. */
  alpha: number;
  /** CSS letter-spacing in px (0 when 'normal'). */
  letterSpacingPx: number;
};

/** One rendered line, with the exact rect the browser gave it (canvas px). */
...
/** Same tolerance the browser uses for sub-pixel line boxes. */
...
/** Transform/clip/filter/blend on this element invalidate flat text placement. */
...
/** Text whose paint we can't restate as a PowerPoint run. */
