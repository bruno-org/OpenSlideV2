...
/**
 * Each page is captured as a background image with the promoted text made
 * transparent, then native text boxes go on top at the measured pixels. Text
 * that cannot be restated faithfully stays painted in the background rather
 * than approximated, so the slide always matches the screen and only its
 * editability degrades.
 *
 * Wrapping is disabled on those boxes: a substituted font can change glyph
 * widths but can never re-flow the deck.
 */
...
/**
 * Alignment survives a wider substituted font by widening the box away from the
 * anchoring edge: a centred line grows around the same axis, a right-aligned
 * one still ends on the same x.
 */
...
// The package skeleton mirrors export-pptx.ts on purpose: that file hardcodes
// one full-bleed picture per slide and is the shipping path for image export,
// so forking the few shared strings beats refactoring a working exporter.
